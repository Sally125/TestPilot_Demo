...
- Name: test.spec.ts >> 每日推送文章列表 >> 环境验证: 页面加载成功
- Location: testpilot-backend\generated\.runtime\4a0e3e9e\test.spec.ts:8:7
...
Locator: getByTestId('page-container')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByTestId('page-container')
...
> 9  |     await expect(page.getByTestId('page-container')).toBeVisible();
  ...
  10 |   });
  11 | 
  12 |   test('TC-002: 文章列表加载性能（响应时间）', async ({ page }) => {
  13 |     const startTime = await page.evaluate(() => performance.now());
  14 |     await page.reload({ waitUntil: 'domcontentloaded' });
  15 |     const endTime = await page.evaluate(() => performance.now());
  16 |     const loadTime = endTime - startTime;
  17 |     expect(loadTime).toBeLessThan(2000);
  18 |   });
  19 | });
```
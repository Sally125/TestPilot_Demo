...
- Location: testpilot-backend\generated\.runtime\959cf1df\test.spec.ts:3:5
...
```ts
  ...
  6  | 
  7  |   // 2. 填入凭证
  8  |   await (page.getByRole('textbox', { name: '邮箱' })).fill('huangkx1225@163.com');
  9  |   await (page.getByRole('textbox', { name: '密码' })).fill('123456');
  10 | 
  11 |   // 3. 点击提交
  12 |   await (page.getByRole('button', { name: '登录' })).click();
  13 | 
  14 |   // 4. 等待登录成功
> 15 |   await page.waitForURL('**http://localhost:3001/dashboard**', { timeout: 10000 });
     |              ^ TimeoutError: page.waitForURL: Timeout 10000ms exceeded.
  16 | 
  17 |   // 5. 保存会话快照
